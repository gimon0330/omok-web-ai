...
def teacher_move(board, player):
    ...
    if len(scored) > 1 and random.random() < 0.12:
...
def generate_games(num_games, max_moves):
    ...
    for game_idx in range(num_games):
        ...
        for _ in range(max_moves):
            ...
            xs.append(encode_board(board, player))
            ys.append(r * SIZE + c)
...
def main():
    ...
    parser.add_argument("--out", type=str, default="training/data/policy_data.npz")
    ...
    x, y = generate_games(args.games, args.max_moves)
