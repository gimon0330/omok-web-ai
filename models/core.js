const GomokuCore = (() => {
  ...
  function alphaBetaMove(board, depth) {
    const opening = openingMove(board);
    if (opening) return opening;
  ...
  function quiescence(board, alpha, beta, player, tactical, ply) {
    ...
    const forcing = candidates.filter(m => {
      const attack = moveThreatScore(board, m.r, m.c, player);
      const defense = moveThreatScore(board, m.r, m.c, player === AI ? HUMAN : AI);
      return attack >= 120_000 || defense >= 180_000;
    }).slice(0, 10);
  ...
  function threatClass(board, r, c, player) {
    ...
    if (four >= 2) return 7;
    if (four >= 1 && openThree >= 1) return 6;
    if (openThree >= 2) return 5;
  ...
  function localMoveScore(board, r, c, player) {
    ...
    if (isWin(board, r, c, player)) score += WIN;
    for (const [dr, dc] of DIRS) score += localLineScore(board, r, c, dr, dc, player);
